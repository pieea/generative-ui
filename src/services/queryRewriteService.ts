...
3. **두 인물 쿼리 패턴** (반드시 3개 쿼리로 확장):
   - "강말금 봉태규" → "강말금"(people) + "봉태규"(people) + "강말금 봉태규"(news)
   - "손흥민 이강인" → "손흥민"(people) + "이강인"(people) + "손흥민 이강인"(news)
   - "유재석 박명수" → "유재석"(people) + "박명수"(people) + "유재석 박명수"(news)
   - 두 인물 이름이 함께 있으면 각 인물의 프로필 + 공통 관련 뉴스
   - 공백이나 쉼표로 구분된 두 이름 패턴을 감지

4. 인물 이름 감지 규칙:
   - 한글 2~4글자 이름 (예: 예지원, 손흥민, 이병헌, 강말금, 봉태규)
   - 장소/지역명 + 인물 조합 시 인물 쿼리 분리
   - 인물 + 이슈/사건 조합 시 인물 쿼리 분리
   - **두 인물 이름이 함께 있으면 3개 쿼리로 확장**
...
**두 인물 쿼리 패턴: 반드시 3개 쿼리로 확장!**
예: "강말금 봉태규" → ["강말금"(people), "봉태규"(people), "강말금 봉태규"(news)]
예: "손흥민 이강인" → ["손흥민"(people), "이강인"(people), "손흥민 이강인"(news)]`,
        },
        {
          role: 'user',
          content: query,
